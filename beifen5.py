    ...
    for each_list in car:
        if each_list[3] != 2:
            ...
            for i in range(0, len(shortestPath) - 1):
                edge = h.getEdge(shortestPath[i], shortestPath[i + 1])
                Road = edge.getRoadlabel()
                single_road.append(Road)
            single_road.insert(0, each_list[0])
            if each_list[2] >= each_list[1]:
                single_road.insert(1, each_list[1] + 270)
            else:
                single_road.insert(1, each_list[1] + 360)
            all_path.append(single_road)
...
def GenerateMatrix(cross, road):
    ...
    for i in range(len(road)):
        if road[i][6] == 1:
            Matrix[road[i][4] - 1][road[i][5] - 1] = road[i][1] / (road[i][3] * 3) + (5 - road[i][2] / 2)
            Matrix[road[i][5] - 1][road[i][4] - 1] = road[i][1] / (road[i][3] * 3) + (5 - road[i][2] / 2)
    Nodes = []
    for i in range(0, len(cross)):
...
def find_shortest_path(graph, start, end, path=[]):
    ...
    for node in graph.neighboringVertices(start):
        ...
        if node not in path:
            ...
            if newpath:
                if not shortest or length1(newpath) < length1(shortest):
                    shortest = newpath
        else:
            continue
    return shortest
...
def length1(a, b):
    edge = g.getEdge(a, b)
    length = int(edge.getRoadlength()/edge.getRoadchannel)
    return length
